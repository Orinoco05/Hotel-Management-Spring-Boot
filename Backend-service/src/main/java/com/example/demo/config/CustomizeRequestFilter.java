...
@Component
@Slf4j(topic = "CUSOMIZE-REQUEST-FILTER")
public class CustomizeRequestFilter extends OncePerRequestFilter {

    private final JwtService jwtService;
    private final CustomerDetailService serviceDetail;

    public CustomizeRequestFilter(JwtService jwtService, CustomerDetailService serviceDetail) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (StringUtils.hasLength(authHeader) && authHeader.startsWith("Bearer ")) {
            ...
            UserDetails user = serviceDetail.CustomerDetailService().loadUserByUsername(username);
